...
module Jekyll
  module Tumblr
    ...
    def self.cached_open(url)
      tmpfile_path = "tmp/" + url.gsub(/[^a-z0-9]/,'-').gsub(/-+/,'-')
      if FileTest.exists?(tmpfile_path)
        open(tmpfile_path)
      else
        open(url).tap do |content|
    ...
    def self.process(url, format = "html", grab_images = false,
      ...
      begin
        ...
        feed = cached_open(url + "?num=#{per_page}&start=#{current_page * per_page}")
      ...
      posts.each do |post|
        ...
        File.open("#{BASE_PATH}/#{post[:name]}", "w") do |f|
    ...
    def self.post_to_hash(post, format)
      ...
      slug = title.downcase.strip.gsub(' ', '-').gsub(/[^\w-]/, '')[0,10]
      {
    ...
    def self.rewrite_urls_and_redirects(posts)
      ...
      urls = Hash[posts.map { |post|
        ...
        File.open("#{BASE_PATH}/#{post[:name]}", "w")
        tumblr_url = URI.parse(post[:slug]).path
        jekyll_url = Jekyll::Post.new(site, dir, "", "" + post[:name]).url
...
Jekyll::Tumblr.process("http://nick.zadrozny.com", "html", true, true, true)
